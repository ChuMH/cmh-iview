<template>
  <div>
    <div>
      起始用户id：<Input v-model="startModel" style="width: 200px"/>
      结束用户id：<Input v-model="endModel" style="width: 200px"/>
...
export default {
  ...
  methods:{
    findList(){
      let resultList = [];
      this.$axios.post("/sys/shortestPath",{
        startId:this.startModel,
        targetId:this.endModel
      }).then(result=>{
